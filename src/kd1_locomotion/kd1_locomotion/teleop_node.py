...
class TeleopNode(Node):
    def __init__(self):
        ...
        self.max_speed_turn = 140.0
        self.max_speed_straight = 100.0
    ...
    def publish_command(self, left, right, label):
        ...
        if label != "STOP":
            if self.motion_start_time is None:
                ...
                #     stop_msg.data = [0.0, 0.0, self.standard_servo_angle]
        ...
        msg.data = [left, right, self.standard_servo_angle]
    ...
    def keyboard_listener(self):
        while True:
            ...
            if key == '\x1b[A':  # up arrow
                self.publish_command(self.max_speed_straight, self.max_speed_straight, "FORWARD")
            elif key == '\x1b[B':  # down arrow
                self.publish_command(-self.max_speed_straight, -self.max_speed_straight, "BACKWARD")
            elif key == '\x1b[C':  # right arrow
                self.publish_command(self.max_speed_turn, -self.max_speed_turn, "RIGHT")
            elif key == '\x1b[D':  # left arrow
                self.publish_command(-self.max_speed_turn, self.max_speed_turn, "LEFT")
            elif key == ' ':
                self.publish_command(0.0, 0.0, "STOP")
            elif key == 'q':
